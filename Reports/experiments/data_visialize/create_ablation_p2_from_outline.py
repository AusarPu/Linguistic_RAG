...
def plot_p2_ablation(output_dir: str, output_name: str) -> str:
    datasets = ["HotpotQA", "TriviaQA"]
    r1_acc = [0.71 * 100, 0.73 * 100]
    r2_acc = [0.75 * 100, 0.85 * 100]
    ...
    ax.set_title("Ablation: Intent Alignment (P2) Effectiveness", fontweight="bold")

    b1 = ax.bar(x - width/2, r1_acc, width, label="R1: Naive RAG (P1)", color=c1, alpha=0.85)
    b2 = ax.bar(x + width/2, r2_acc, width, label="R2: P1 + P2", color=c2, alpha=0.85)
...
def main():
    ...
    output_name = "ablation_p2_effectiveness.png"
    p = plot_p2_ablation(output_dir, output_name)
    print(p)
