...
# --- 用于独立测试此 RAG Pipeline 的示例 ---
async def main_test_full_rag_flow():
    # 确保所有VLLM服务 (Rewriter, Reranker, Generator) 都已启动
    # 确保 KnowledgeBase 的索引文件已生成并可访问

    # 1. 初始化 KnowledgeBase
    kb = None
    try:
        logger.info("正在为RAG流程测试初始化 KnowledgeBase...")
        kb = KnowledgeBase()  # 它会在内部加载所有索引
        logger.info("KnowledgeBase 初始化成功。")
    except Exception as e:
        logger.critical(f"RAG流程测试：KnowledgeBase 初始化失败: {e}", exc_info=True)
        return

    # 2. 定义测试查询和历史
    test_query = "索绪尔的语言符号理论有哪些核心观点，他对任意性的具体阐述是什么？"
    test_history_openai: List[Dict[str, str]] = [
        {"role": "user", "content": "你好，我想了解一下索绪尔。"},
        {"role": "assistant",
         "content": "索绪尔是现代语言学的重要人物，他的《普通语言学教程》影响深远。您想了解他理论的哪方面呢？"}
    ]

    logger.info(f"\n\n=========== 开始独立测试 RAG Pipeline Flow (execute_rag_flow) ===========")
    logger.info(f"测试查询: \"{test_query}\"")

    final_answer_collected = ""
    reasoning_collected = ""

    # 3. 调用 execute_rag_flow 并处理事件
    async for event in execute_rag_flow(
            user_query=test_query,
            chat_history_openai=test_history_openai,
            kb_instance=kb
    ):
        event_type = event.get("type")
        logger.info(f"[TEST_CALLBACK] Received Event Type: {event_type}")
        # logger.debug(f"[TEST_CALLBACK] Event Data: {json.dumps(event, ensure_ascii=False, indent=2, default=str)}")

        if event_type == "status":
            logger.info(f"  Status Update: [{event.get('stage')}] {event.get('message')}")
        elif event_type == "rewritten_query_result":
            logger.info(f"  Rewritten Query: {event.get('rewritten_text')}")
        elif event_type == "retrieved_chunks_preview":
            logger.info(
                f"  Retrieved {event.get('count')} chunks initially. Preview (first few): {event.get('preview')}")
        elif event_type == "reranked_context_for_display":
            logger.info(f"  Reranked Context for UI (Top {len(event.get('chunks', []))}):")
            for chunk_info in event.get('chunks', []):
                logger.info(
                    f"    - ID: {chunk_info.get('chunk_id')}, Score: {chunk_info.get('rerank_score', 'N/A'):.4f}, "
                    f"Text: \"{chunk_info.get('text_preview', '')}\"")
        elif event_type == "llm_input_preview":
            logger.info(
                f"  Preview of data sent to Generator: SysPromptUsed={event.get('system_prompt_used')}, UserContentLen={event.get('user_content_length')}")
        elif event_type == "reasoning_delta":
            print(f"{event.get('text', '')}", end="", flush=True)  # 流式打印思考过程
            reasoning_collected += event.get("text", "")
        elif event_type == "content_delta":
            print(f"{event.get('text', '')}", end="", flush=True)  # 流式打印答案
            final_answer_collected += event.get("text", "")
        elif event_type == "final_answer_complete":
            logger.info(
                f"\n  Final Full Answer Received (length: {len(event.get('full_text', ''))}). Context Chunks Used: {event.get('final_context_chunk_ids')}")
        elif event_type == "error":
            logger.error(f"  Pipeline Error at stage '{event.get('stage')}': {event.get('message')}")
            break
        elif event_type == "pipeline_end":
            logger.info(f"  Pipeline Ended. Reason: {event.get('reason')}")
            break

    logger.info("\n\n--- RAG Pipeline 独立测试执行完毕 ---")
    if reasoning_collected:
        logger.info(f"--- 累积的思考过程 ---\n{reasoning_collected}")
    if final_answer_collected:
        logger.info(f"--- 累积的最终答案 ---\n{final_answer_collected}")

    logger.info("=========== RAG Pipeline Flow 测试结束 ===========")


if __name__ == '__main__':
    # 配置基础日志，以便在直接运行此文件时能看到输出
    if not logging.getLogger().hasHandlers():
        ...
        logging.getLogger().setLevel(logging.INFO)  # 设置为你希望的级别，例如 DEBUG
        # logging.getLogger("aiohttp").setLevel(logging.WARNING) # 减少aiohttp的日志

    # 确保所有VLLM服务 (Rewriter, Reranker, Generator) 都已启动
    # 确保 KnowledgeBase 的索引文件已生成并可访问
    # 确保 script/config.py 中的所有路径和API配置都正确
    asyncio.run(main_test_full_rag_flow())