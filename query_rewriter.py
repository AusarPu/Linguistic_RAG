...
def generate_rewritten_query(
    ...
    # 构造一个临时的消息列表用于重写任务
    # 可以尝试不同的指令模板，看哪个效果好
    rewrite_instruction = f"""你的角色是专门为 **语言学知识库** 优化用户查询的专家助手。你将收到一段对话历史和用户的最新查询。你的任务是将这个最新查询改写成一个独立的、清晰的、精确的搜索查询语句，这个语句要适合从一个专门的语言学数据库中检索信息。

**具体指令：**

1.  **分析上下文：** 仔细检查提供的对话历史，以理解当前的讨论主题，并解决用户最新查询（`'{user_input}'`）中的任何不明确之处。
...
现在，请基于提供的对话历史和用户最新查询：`'{user_input}'`，开始生成优化后的查询语句。"""
    ...
    rewrite_prompt = tokenizer.apply_chat_template(rewrite_messages, tokenize=False, add_generation_prompt=False) # 假设需要助手提示
